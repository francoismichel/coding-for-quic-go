...
func ListenAddr(addr string, tlsConf *tls.Config, config *Config) (Listener, error) {
	return listenAddr(addr, tlsConf, config)
}
...
func ListenAddrEarly(addr string, tlsConf *tls.Config, config *Config) (EarlyListener, error) {
	s, err := listenAddr(addr, tlsConf, config)
	if err != nil {
		return nil, err
	}
...
func listenAddr(addr string, tlsConf *tls.Config, config *Config) (*baseServer, error) {
...
func ListenEarly(conn net.PacketConn, tlsConf *tls.Config, config *Config) (EarlyListener, error) {
	s, err := listen(conn, tlsConf, config)
	if err != nil {
		return nil, err
	}
	s.acceptEarlySessions = true
...
func listen(conn net.PacketConn, tlsConf *tls.Config, config *Config) (*baseServer, error) {
